...
import { CreateBookingDto, UpdateBookingDto, BookingDto } from '@frequenc/shared';
...
export class BookingsController {
  ...
  @ApiResponse({ status: 200, description: 'Bookings retrieved successfully', type: [BookingDto] })
  async getUserBookings(@Request() req, @Query() query: { page?: number; limit?: number; status?: string }) {
    return firstValueFrom(
      this.bookingsService.send('bookings.get-user-bookings', { userId: req.user.id, ...query })
    );
  }
  ...
  async createBooking(@Request() req, @Body() createBookingDto: CreateBookingDto) {
    return firstValueFrom(
      this.bookingsService.send('bookings.create', { userId: req.user.id, ...createBookingDto })
    );
  }
  ...
  async updateBooking(@Request() req, @Param('id') id: string, @Body() updateBookingDto: UpdateBookingDto) {
    return firstValueFrom(
      this.bookingsService.send('bookings.update', { id, userId: req.user.id, ...updateBookingDto })
    );
  }
  ...
  async getTimeSlots(@Query() query: { venueId: string; courtId: string; date: string }) {
    return firstValueFrom(
  ...
  async checkAvailability(@Body() availabilityDto: { venueId: string; courtId: string; date: string; timeSlot: string }) {
    return firstValueFrom(
  ...
  async confirmBooking(@Request() req, @Param('id') id: string) {
    return firstValueFrom(
      this.bookingsService.send('bookings.confirm', { id, userId: req.user.id })
    );
  }
} 