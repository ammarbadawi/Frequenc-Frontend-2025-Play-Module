...
export class GamesController {
  ...
  @ApiResponse({ status: 200, description: 'Games retrieved successfully', type: [GameDto] })
  async getAvailableGames(@Query() query: { venueId?: string; sportType?: string; date?: string }) {
    return firstValueFrom(
      this.gamesService.send('games.get-available', query)
    );
  }
  ...
  async createGame(@Request() req, @Body() createGameDto: CreateGameDto) {
    return firstValueFrom(
      this.gamesService.send('games.create', { userId: req.user.id, ...createGameDto })
    );
  }
  ...
  async leaveGame(@Request() req, @Param('id') id: string) {
    return firstValueFrom(
      this.gamesService.send('games.leave', { gameId: id, userId: req.user.id })
    );
  }
  ...
  @ApiResponse({ status: 200, description: 'Game details retrieved successfully', type: GameDto })
  ...
  async updateGameStatus(@Request() req, @Param('id') id: string, @Body() statusDto: { status: string }) {
    return firstValueFrom(
      this.gamesService.send('games.update-status', { gameId: id, userId: req.user.id, ...statusDto })
    );
  }
} 